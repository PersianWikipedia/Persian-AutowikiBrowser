        ...
        private void RunSkipChecks()
        {
            ...
            if (!Abort)
            {
                ...
                if (Skippable)
                {
                    ...
                    {
                        SkipPage("فقط تغییرات زیباسازی کدهای ویکی انجام شد");
                        return;
                    }
            ...
            try
            {
                ...
                if (TheArticle.OriginalArticleText.Equals(txtEdit.Text))
                {
                    webBrowser.Document.Write(
                        @"<span dir='rtl'><h2 style='padding-top: .5em;
padding -bottom: .17em;
border-bottom: 1px solid #aaa;
font-size: 150%;'>بدون تغییر</h2><p>دکمه «رها» را بزنید تا صفحهٔ بعدی بررسی شود.</p></span>");
                }
                else
                {
        ...
        private void Save()
        {
                 ...
                 DialogResult.Yes))
            {
                ...
                SkipPage("چیزی برای ذخیره وجود ندارد صفحهٔ خالی");
        }
        ...
        private void chkAppend_CheckedChanged(object sender, EventArgs e)
        {
            txtAppendMessage.Enabled = rdoAppend.Enabled = rdoPrepend.Enabled =
                udNewlineChars.Enabled  = lblNewlineCharacters.Enabled = chkAppendMetaDataSort.Enabled = chkAppend.Checked;
        }
        ...
        private void NudgeTimer_Tick(object sender, NudgeTimer.NudgeTimerEventArgs e)
        {
            ...
            if (BotMode)
            {
                ...
                {
                    SameArticleNudges = 0;
                    SkipPage("خطا در ذخیره صفحه، از نو در حال ذخیره");
                }
                else
                {
                    SameArticleNudges++;
        ...
        public void RTLMessageBox (string message, string title)
        {
            MessageBox.Show(message, title, MessageBoxButtons.OK, MessageBoxIcon.None, MessageBoxDefaultButton.Button1, MessageBoxOptions.RtlReading);
        }
        private void imgAbar_Click(object sender, EventArgs e)
        {
            string test_txt;
            try
            { 
            ...
            using (System.IO.StreamWriter writer = new System.IO.StreamWriter("input.txt"))
                ...
                {
                    RTLMessageBox("ممکن است نرم‌افزار node نصب نشده باشد یا فایل fa_bot.js در پوشهٔ ویرایشگر خودکار نباشد.", "خطا در اجرای ابرابزار");
                }
        ...
        {
            if (txtEdit.RightToLeft == System.Windows.Forms.RightToLeft.No)
            {
                txtEdit.RightToLeft = System.Windows.Forms.RightToLeft.Yes;
            }
            else
            {
